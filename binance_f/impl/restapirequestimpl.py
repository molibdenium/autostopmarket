...
class RestApiRequestImpl(object):
	...
	def get_leverage_bracket(self, symbol): #createdbyme
		builder = UrlParamsBuilder()
		builder.put_url("symbol", symbol)
	...
	def post_change_position_mode(self, dualSidePosition):
		builder = UrlParamsBuilder()
		...
		def parse(json_wrapper):
			result = list()	
			element = PositionMode.json_parse(json_wrapper)
			result.append(element)
			return element
	...
	def post_order(self, symbol, side, ordertype, positionSide,
